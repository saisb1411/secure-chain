...
const validateTransaction = (transaction: Transaction, aUnspentTxOuts: UnspentTxOut[]): boolean => {
    ...
    if (totalTxOutValues > MAX_TRANSACTION_LIMIT) {
        console.log('Transaction blocked: exceeds maximum allowed amount');
        return false;
    }
...
const validateCoinbaseTx = (transaction: Transaction, blockIndex: number): boolean => {
    ...
    if (transaction.txOuts[0].amount < BASE_REWARD) {
...
const getCoinbaseTransaction = (address: string, blockIndex: number): Transaction => {
    ...
    t.txIns = [txIn];
    const reward = BASE_REWARD + (blockIndex * 2); 
